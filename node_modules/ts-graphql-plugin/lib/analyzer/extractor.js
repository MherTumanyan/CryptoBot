...
class Extractor {
    ...
    extract(files, tagName) {
        ...
        files.forEach(fileName => {
            const nodes = this._helper
                ...
                .filter(node => (tagName ? ts_ast_util_1.isTagged(node, tagName) : true));
            nodes.forEach(node => {
        ...
        return results.map(result => {
            if (!result.resolevedTemplateInfo)
                return result;
            try {
                const rawDocumentNode = graphql_1.parse(result.resolevedTemplateInfo.combinedText);
                if (!this._removeDuplicatedFragments) {
                    return Object.assign(Object.assign({}, result), { documentNode: rawDocumentNode });
                }
                const duplicatedInfo = gql_ast_util_1.detectDuplicatedFragments(rawDocumentNode);
                ...
                const documentNode = graphql_1.parse(updatedResolvedInfo.combinedText);
                return Object.assign(Object.assign({}, result), { documentNode, resolevedTemplateInfo: updatedResolvedInfo });
            }
            catch (error) {
                return Object.assign(Object.assign({}, result), { graphqlError: error });
            }
        });
    }
    ...
    getDominantDefiniton(result) {
        let type;
        const definedFragmentNames = [];
        const referencedFragmentNames = [];
        let operationName;
        language_1.visit(result.documentNode, {
    ...
    toManifest(extractResults, tagName = '') {
        const documents = extractResults
            ...
            return {
                ...
                operationName,
                fragmentName,
                body: graphql_1.print(r.documentNode),
                tag: tagName,
