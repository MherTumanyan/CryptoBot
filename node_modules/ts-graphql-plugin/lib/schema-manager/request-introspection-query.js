...
function requestIntrospectionQuery(options) {
    const headers = Object.assign({ 'Content-Type': 'application/json', 'Content-Length': INTROSPECTION_QUERY_LENGTH, 'User-Agent': 'ts-graphql-plugin' }, options.headers);
    return new Promise((resolve, reject) => {
        const uri = url_1.parse(options.url);
            ...
            res.on('end', () => {
                if (!res.statusCode || res.statusCode < 200 || res.statusCode > 300) {
                    ...
                    try {
                        ...
                        resolve(graphql_1.buildClientSchema(result.data));
                    }
                    catch (e) {
                        reject(e);
                    }
