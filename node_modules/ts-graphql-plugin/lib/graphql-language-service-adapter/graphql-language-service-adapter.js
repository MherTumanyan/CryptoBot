...
class GraphQLLanguageServiceAdapter {
    ...
    getCompletionAtPosition(delegate, ...args) {
        return get_completion_at_position_1.getCompletionAtPosition(this._analysisContext, delegate, ...args);
    }
    getSemanticDiagnostics(delegate, ...args) {
        return get_semantic_diagonistics_1.getSemanticDiagnostics(this._analysisContext, delegate, ...args);
    }
    getQuickInfoAtPosition(delegate, ...args) {
        return get_quick_info_at_position_1.getQuickInfoAtPosition(this._analysisContext, delegate, ...args);
    }
    ...
    _findTemplateNode(fileName, position) {
        ...
        else if (typescript_1.default.isTemplateHead(foundNode)) {
        ...
        if (this._tagCondition && !ts_ast_util_1.isTagged(node, this._tagCondition)) {
            return;
        }
        return node;
    }
    _findTemplateNodes(fileName) {
        ...
        const nodes = allTemplateStringNodes.filter(n => {
            if (!this._tagCondition)
                return true;
            return ts_ast_util_1.isTagged(n, this._tagCondition);
        });
        return nodes;
    }
    _resolveTemplateInfo(fileName, node) {
        ...
        try {
            const documentNode = graphql_1.parse(resolvedInfo.combinedText);
            const duplicatedFragmentInfoList = gql_ast_util_1.detectDuplicatedFragments(documentNode);
