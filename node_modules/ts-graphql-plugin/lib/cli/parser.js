...
function createParser(parseOptions) {
    ...
    const parse = (rawArguments = process.argv) => {
        const showHelp = () => {
            ...
            Object.entries(parseOptions.options).forEach(([name, value]) => {
                ...
                if (value.description) {
                    line += string_util_1.pad(' ', 42 - line.length) + value.description;
                }
        ...
        if (!subCommandName) {
            return Object.assign({ _: args, options: baseOptions }, funcs);
        }
        else if (!parseOptions.commands[subCommandName]) {
            return Object.assign({ _: args, options: baseOptions, errors: {
                    unknownCommand: subCommandName,
                } }, funcs);
        }
        else {
            return Object.assign({ _: [], options: baseOptions, command: {
                    [subCommandName]: {
                        _: args,
