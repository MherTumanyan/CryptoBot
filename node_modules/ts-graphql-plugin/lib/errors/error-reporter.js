...
const lineMark = (line, width) => {
    ...
    return string_util_1.color.invert(string_util_1.pad(' ', width - strLine.length) + strLine) + ' ';
};
const lineMarkForUnderline = (width) => {
    return string_util_1.color.invert(string_util_1.pad(' ', width)) + ' ';
};
class ErrorReporter {
    ...
    _outputErrorWithLocation(error) {
        ...
        const startLC = string_util_1.pos2location(content, start);
        const endLC = string_util_1.pos2location(content, end);
            ...
            if (i === 0) {
                if (startLC.line === endLC.line) {
                    outputs.push(lineMarkForUnderline(lineMarkerWidth) +
                        string_util_1.pad(' ', startLC.character) +
                        string_util_1.color.red(string_util_1.pad('~', endLC.character - startLC.character)));
                }
                else {
                    outputs.push(lineMarkForUnderline(lineMarkerWidth) +
                        string_util_1.pad(' ', startLC.character) +
                        string_util_1.color.red(string_util_1.pad('~', lines[i].length - startLC.character)));
                }
            }
            else if (i === lines.length - 1) {
                outputs.push(lineMarkForUnderline(lineMarkerWidth) + string_util_1.color.red(string_util_1.pad('~', endLC.character)));
            }
            else {
                outputs.push(lineMarkForUnderline(lineMarkerWidth) + string_util_1.color.red(string_util_1.pad('~', lines[i].length)));
            }
        }
